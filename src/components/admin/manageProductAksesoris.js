...
class CustomPaginationActionsTable extends React.Component {
  ...
  valueHandlerEdit = () => {
  var value = this.state.selectedFileEdit ? this.state.selectedFileEdit.name : 'Pict a Picture'
  return value
  }
  ...
  onBtnAdd = () => {
    if(
      ...
      this.state.selectedFile === null){
        ...
        var newData = {nama_aksesoris : this.refs.nama.value , harga_aksesoris : this.refs.harga.value ,
          discount_aksesoris : this.refs.diskon.value , category_aksesoris: this.refs.category.value ,deskripsi_aksesoris : this.refs.deskripsi.value }

        var fd = new FormData()
        fd.append('data' , JSON.stringify(newData))
        fd.append('image', this.state.selectedFile,this.state.selectedFile.nama)
        Axios.post(urlApi+'/aksesori/addaksesoris', fd)
        .then((res) => {
        if(res.data.error){
        this.setState({error : res.data.msg})
        } else {
        swal("Aksesoris Product Added", "New product has been added", "success")
          this.getDataApi()
          this.refs.nama.value=''
          this.refs.harga.value=''
          this.refs.diskon.value=''
  ...
  onBtnSave = () => {
    var data = {
      nama_aksesoris : this.refs.namaEdit.value ? this.refs.namaEdit.value : this.state.editItem.nama_aksesoris ,
      harga_aksesoris : this.refs.hargaEdit.value ? this.refs.hargaEdit.value : this.state.editItem.harga_aksesoris,
      discount_aksesoris : this.refs.diskonEdit.value ? this.refs.diskonEdit.value : this.state.editItem.discount_aksesoris,
      category_aksesoris : this.refs.kategoriEdit.value ? this.refs.kategoriEdit.value : this.state.editItem.category_aksesoris,
      deskripsi_aksesoris : this.refs.deskripsiEdit.value ? this.refs.deskripsiEdit.value : this.state.editItem.deskripsi_aksesoris
    }
    if(this.state.selectedFileEdit){
      var fd = new FormData()
      fd.append('edit', this.state.selectedFileEdit)
      fd.append('data' , JSON.stringify(data))
      fd.append('imageBefore' , this.state.editItem.image_aksesoris)
      Axios.put('http://localhost:2008/aksesori/editaksesoris/'+ this.state.editItem.id,fd)
        .then((res) => {
      ...
      Axios.put('http://localhost:2008/aksesori/editaksesoris/'+ this.state.editItem.id_aksesoris, data)
      .then((res) =>{
        swal("Aksesoris Product Edited", res.data, "success")  
        this.getDataApi()
        this.setState({modal:false})
      })
      .catch((err) =>{
  ...
  onBtnDelete = (id) => {
    Axios.delete(urlApi + '/aksesori/deleteaksesoris/' + id)
    .then((res) => {
        ...
        return (
            <TableRow key={val.id}>
                <TableCell>{val.id}</TableCell>
                  ...
                  <TableCell>Rp. {val.harga_aksesoris}</TableCell>
                  <TableCell>{val.discount_aksesoris}%</TableCell>
                  <TableCell>{val.category_aksesoris}</TableCell>
                  <TableCell><img src={`http://localhost:2008/`+val.image_aksesoris} alt='..' width='50px'/></TableCell>
                  <TableCell>{val.deskripsi_aksesoris}</TableCell>
                  <TableCell>
      ...
      return (
        <div className='container' style={{marginTop:'80px'}}>
          ...
          <div className = 'row mt-3 mb-2'>
              ...
              <div className= 'col-md-4'>
                  <select className="form-control mb-1" ref="category">{this.dropdownCategory()}</select>
          ...
          <div>
            <Modal isOpen={this.state.modal} toggle={() => this.setState({modal:false})} className={this.props.className}>
              <ModalHeader toggle={() => this.setState({modal:false})}>Edit Produk ~ {this.state.editItem.nama_aksesoris}</ModalHeader>
              <ModalBody>
                <div className = "row">
                  <div className="col-md-3">
                    <img src={'http://localhost:2008/'+this.state.editItem.image_aksesoris} width='100%' alt='broken' />
                  ...
                  <div className="col-md-9">
                    <input type = "text" className='form-control' ref='namaEdit' placeholder={this.state.editItem.nama_aksesoris}/>
                    <input type = "number" className='form-control mt-3' ref='hargaEdit' placeholder={this.state.editItem.harga_aksesoris}/><br/>
                    <input type = "number" className='form-control' ref='diskonEdit' placeholder={this.state.editItem.discount_aksesoris}/>
                    <select className='form-control mt-3' ref='kategoriEdit'>
                      {this.dropdownCategory()}
                    </select>
                    <textarea type = "text" row='3' className='form-control mt-3' ref='deskripsiEdit' placeholder={this.state.editItem.deskripsi_aksesoris}/>
                  </div>
                </div>
              </ModalBody>
              <ModalFooter>
