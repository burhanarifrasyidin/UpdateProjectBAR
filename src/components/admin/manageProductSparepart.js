...
class CustomPaginationActionsTable extends React.Component {
  ...
  onBtnAdd = () => {
    if(
      ...
      this.state.selectedFile === null){
        ...
        var newData = {nama_sparepart : this.refs.nama.value , harga_sparepart : this.refs.harga.value ,
          discount_sparepart : this.refs.diskon.value , category_sparepart: this.refs.category.value ,deskripsi_sparepart : this.refs.deskripsi.value }

        var fd = new FormData()
        fd.append('data' , JSON.stringify(newData))
        fd.append('image', this.state.selectedFile,this.state.selectedFile.nama)
        Axios.post(urlApi+'/sparepart/addsparepart', fd)
        .then((res) => {
        if(res.data.error){
        this.setState({error : res.data.msg})
        } else {
        swal("Sparepart Added", "New product has been added", "success")
          this.getDataApi()
          this.refs.nama.value=''
          this.refs.harga.value=''
          this.refs.diskon.value=''
  ...
  onBtnSave = () => {
    var data = {
      nama_sparepart : this.refs.namaEdit.value ? this.refs.namaEdit.value : this.state.editItem.nama_sparepart ,
      harga_sparepart : this.refs.hargaEdit.value ? this.refs.hargaEdit.value : this.state.editItem.harga_sparepart,
      discount_sparepart : this.refs.diskonEdit.value ? this.refs.diskonEdit.value : this.state.editItem.discount_sparepart,
      category_sparepart : this.refs.kategoriEdit.value ? this.refs.kategoriEdit.value : this.state.editItem.category_sparepart,
      deskripsi_sparepart : this.refs.deskripsiEdit.value ? this.refs.deskripsiEdit.value : this.state.editItem.deskripsi_sparepart
    }
    if(this.state.selectedFileEdit){
      var fd = new FormData()
      fd.append('edit', this.state.selectedFileEdit)
      fd.append('data' , JSON.stringify(data))
      fd.append('imageBefore' , this.state.editItem.image_sparepart)
      Axios.put('http://localhost:2008/sparepart/editsparepart/'+ this.state.editItem.id_sparepart,fd)
        .then((res) => {
          swal("Sparepart Edited", res.data, "success")  
          this.getDataApi()
      ...
      Axios.put('http://localhost:2008/sparepart/editsparepart/'+ this.state.editItem.id_sparepart, data)
      .then((res) =>{
        swal("Sparepart Edited", res.data, "success")  
        this.getDataApi()
        this.setState({modal:false})
  ...
  onBtnDelete = (id_sparepart) => {
    Axios.delete(urlApi + '/sparepart/deletesparepart/' + id_sparepart)
    .then((res) => {
  ...
  renderJsx = () => {
    var jsx = this.state.rows.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((val) => {
        return (
            <TableRow key={val.id_sparepart}>
                <TableCell>{val.id_sparepart}</TableCell>
                  <TableCell component="th" scope="row">
                    {val.nama_sparepart}
                  </TableCell>
                  <TableCell>Rp. {val.harga_sparepart}</TableCell>
                  <TableCell>{val.discount_sparepart}%</TableCell>
                  <TableCell>{val.category_sparepart}</TableCell>
                  <TableCell><img src={`http://localhost:2008/`+val.image_sparepart} alt='..' width='50px'/></TableCell>
                  <TableCell>{val.deskripsi_sparepart}</TableCell>
                  <TableCell>
                 ...
                 <Button animated color='red' style={{borderRadius:'40px',marginTop:'2px'}} onClick={() => this.onBtnDelete(val.id_sparepart)}>
    ...
    if(this.props.role === 'admin'){
      return (
        ...
        <Modal isOpen={this.state.modal} toggle={() => this.setState({modal:false})} className={this.props.className}>
          <ModalHeader toggle={() => this.setState({modal:false})}>Edit Produk ~ {this.state.editItem.nama_sparepart}</ModalHeader>
          <ModalBody>
            <div className = "row">
              <div className="col-md-3">
                <img src={'http://localhost:2008/'+this.state.editItem.image_sparepart} width='100%' alt='broken' />
              ...
              <div className="col-md-9">
                <input type = "text" className='form-control' ref='namaEdit' placeholder={this.state.editItem.nama_sparepart}/>
                <input type = "number" className='form-control mt-3' ref='hargaEdit' placeholder={this.state.editItem.harga_sparepart}/><br/>
                <input type = "number" className='form-control' ref='diskonEdit' placeholder={this.state.editItem.discount_sparepart}/>
                <select className='form-control mt-3' ref='kategoriEdit'>
                  {this.dropdownCategory()}
                </select>
                <textarea type = "text" row='3' className='form-control mt-3' ref='deskripsiEdit' placeholder={this.state.editItem.deskripsi_sparepart}/>
              </div>
            </div>
          </ModalBody>
          <ModalFooter>
