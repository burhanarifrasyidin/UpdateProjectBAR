...
class AksesorisDetail extends React.Component{
    state = {aksesoris : {}}
    componentDidMount(){
        this.getDataApi()
    }
    ...
    getDataApi = () => {
        ...
        Axios.get(urlApi + '/aksesori/aksesoris-detail/' + idUrl)
        .then((res) => {
            this.setState({aksesoris : res.data[0]})
        })
    ...
    addBtnWish = () => {
        var newData = {
            ...
            nama : this.state.aksesoris.nama,
            harga : this.state.aksesoris.harga,
            deskripsi : this.state.aksesoris.deskripsi,
            qty : 1,
        }
        Axios.get(urlApi + '/wishlist?nama=' + this.state.aksesoris.nama + '&idUser=' + this.props.id).then((res) => {
            if(res.data.length > 0){
    ...
    render(){
        var{nama_aksesoris,harga_aksesoris,discount_aksesoris,image_aksesoris,deskripsi_aksesoris} = this.state.aksesoris
        return(
            <div className='container' style={{paddingTop:'150px'}}>
                <div  className='row'>
                    <div className='col-md-4'>
                        <div className="card" style={{width: '100%'}}>
                            <img src={`http://localhost:2008/${image_aksesoris}`} className="card-img-top" alt="..." />
                    ...
                    <div className='col-md-8'>
                        <h1 style={{color:'#4C4C4C'}}>{nama_aksesoris}</h1>
                        ...
                        <span style={{fontWeight:'600',fontSize:'15px',color:'#606060',marginLeft:'10px',textDecoration:'line-through'}}> Rp. {harga_aksesoris}</span>
                        <div style={{fontWeight:'700',fontSize:'24px',color:'#FF5722',marginTop:'20px'}}>Rp. {harga_aksesoris-(harga_aksesoris*(discount_aksesoris/100))}</div>
                        ...
                        <div className='row mt-4'>
                            <div className='col-md-8'>
                                <p style={{color:'#606060',fontStyle:'italic'}}>{deskripsi_aksesoris}</p>
                            </div>
                        </div>
                        {
                            this.props.username === '' ?
...
export default connect (mapStateToProps,{cartCount})(AksesorisDetail);